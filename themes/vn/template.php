 ...
 */
/*
function vn_preprocess_user_profile(&$profile) {
  //dpm($profile);
}
*/
 ...
 */
function vn_preprocess_user_picture(&$picture) {
  ...
  if (!empty($picture['account']->realname)) {
    ...
    $userExtendedData = vn_misc_loadUserExtendedData($picture['account']->uid);
 ...
 * @ingroup themeable
 */
function vn_pager_next($variables) {
 ...
 * @ingroup themeable
 */
function vn_pager_previous($variables) {
...
//  }
//  elseif ( $arg_0 == 'node' && (in_array($arg_1, $altered_pager_reviews) || vn_misc_identifyNodeType($arg_1) == 'provider') ) {
 ...
 */
function vn_preprocess_search_results(&$variables) {
...
//  $variables['pager'] = theme('pager', array('tags' => NULL, 'element' => 7));
  ...
  // need prior actions (hack) in function node_search_execute() at node.module
  // v2
  global $vn_num_rows, $vn_limit_rows;
//  dpm('$vn_num_rows =' . $vn_num_rows, '$vn_limit_rows = ' . $vn_limit_rows);
//  dpm($_SESSION['vn_node_search_data']);
//  $page = pager_default_initialize($_SESSION['vn_node_search_data']['vn_num_rows'], $_SESSION['vn_node_search_data']['vn_limit_rows']);
  $page = pager_default_initialize($vn_num_rows, $vn_limit_rows);
 ...
 */
function vn_html_head_alter(&$head_elements) {
...
//      global $user;
//      if ($user->uid == 1) {
//        vn_misc_addMetatag('title', $altered_head_title);
//      }
    }
  }
  //dpm($head_elements);
 ...
 */
function vn_menu_local_tasks(&$variables) {
  $output = '';
...
/*
function vn_preprocess_breadcrumb(&$variables) {
  dpm($variables);
}
*/
 ...
 */
function vn_breadcrumb($variables) {
 ...
 */
function vn_process_page(&$variables) {
  ...
  elseif (arg(0) == 'user' && is_int($arg_uid = arg(1))) {
    ...
    $userExtendedData = vn_misc_loadUserExtendedData($arg_uid);
 ...
 */
function vn_preprocess_node(&$variables) {
 ...
 */
function vn_preprocess_region(&$variables, $hook) {
 ...
 */
function vn_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function vn_process_block(&$variables, $hook) {
 ...
 */
function vn_preprocess_search_block_form(&$vars) {
 ...
 */
function vn_form_alter(&$form, &$form_state, $form_id) {
...
function vn_preprocess_views_view_row_rss(&$vars) {
  ...
  if (isset($node->body['und'][0]['value'])) {
    $rss_teaser = vn_misc_getArticleTeaserData('all', $node->body['und'][0]['value'], $vars['row']->nid, 400, TRUE);
...
//  }
  ...
  if (isset($vars['view']->name) && ($vars['view']->name == 'blog' || $vars['view']->name == 'news') ) {
    $function = 'vn_preprocess_views_view_unformatted'; 
 ...
 * 3. Read carefully, especially within vn_preprocess_html(), there
 ...
 */
function vn_preprocess_html(&$vars) {
  // Load the media queries styles
  // Remember to rename these files to match the names used here - they are
  // in the CSS directory of your subtheme.
//  $media_queries_css = array(
//    'vn.responsive.style.css',
//    'vn.responsive.gpanels.css'
//  );
//  load_subtheme_media_queries($media_queries_css, 'vn');
...
//  );
//  load_subtheme_ie_styles($ie_files, 'vn');
